...
for p in range(20, 100, 20):
    save_dir = 'data/model/bert-base-pas{}'.format(p)
    parser = SDPParser()
    parser.train(train_file='data/semeval15/en.pas.train.{}.conllu'.format(p),
    ...
    parser.evaluate(test_file='data/semeval15/en.id.pas.conllu', bert_path='data/embedding/bert_base_sum/en.id.bert',
                    save_dir=save_dir, logger=logger)
    parser.evaluate(test_file='data/semeval15/en.ood.pas.conllu', bert_path='data/embedding/bert_base_sum/en.ood.bert',
                    save_dir=save_dir, logger=logger)
