        ...
        for line in f:
            ...
            if len(split) > 2:
                ...
                if m == 0:
                    ...
                    continue
                word_to_embed[word] = nd.array(vec)
...
class Token:
    ...
    def get_embedding(self, key=None) -> nd.NDArray:
        if key is not None:
            vector = self._embeddings[key]
            if not isinstance(vector, nd.NDArray):
                vector = nd.array(vector)
            return vector
        if self.final_embedding is not None:
            return self.final_embedding

        if len(self._embeddings) == 1:
            vector = list(self._embeddings.values())[0]
            if not isinstance(vector, nd.NDArray):
                vector = nd.array(vector)
            return vector
        embeddings = []
...
class Sentence:
    ...
    def convert_tag_scheme(self, tag_type: str = 'ner', target_scheme: str = 'iob', source_scheme='iob'):
        ...
        if target_scheme == 'iobes' and tag_type != 'iobes':
...
class NLPTaskDataFetcher:
    ...
    @staticmethod
    def fetch_data(task: NLPTask, data_folder) -> TaggedCorpus:
        """
    ...
    def fetch_column_corpus(
        ...
        sentences_train = NLPTaskDataFetcher.read_column_data(
            os.path.join(data_folder, train_file), column_format, portion=0)
        sentences_test = NLPTaskDataFetcher.read_column_data(
            os.path.join(data_folder, test_file), column_format, portion=2)

        if dev_file is not None:
            sentences_dev = NLPTaskDataFetcher.read_column_data(
                os.path.join(data_folder, dev_file), column_format, portion=1)
        else:
    ...
    def read_column_data(path_to_column_file: str,
        ...
        idx = 0
        sentence = Sentence(idx=idx, portion=portion)
        for line in lines:
            if line.startswith('#'):
                continue
