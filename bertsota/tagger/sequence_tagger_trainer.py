...
from bertsota.tagger.corpus import TaggedCorpus, Sentence
from bertsota.tagger.reduce_lr_on_plateau import ReduceLROnPlateau
...
class SequenceTaggerTrainer:
            ...
            with mx.Context(context if context else mxnet_prefer_gpu()):
                ...
                for epoch in range(0, max_epochs):
                    ...
                    for batch in batches:
                        ...
                        sys.stdout.write("\r%.2f%%" % (batch_no / float(len(batches)) * 100))
                        sys.stdout.flush()
                    ...
                    if self.corpus.test and len(self.corpus.test) and show_test:
                        print('test... ')
    ...
    def evaluate(self, evaluation: List[Sentence], out_path=None, evaluation_method: str = 'F1',
                 embeddings_in_memory: bool = True, cn=False, dump: bool = False):
        ...
        for batch in batches:
            ...
            for sentence in batch:
                ...
                if dump:
                    score, tag_seq, att = self.model.predict_scores(sentence, dump)
                    att_vec.append(att)
                else:
                ...
                for (token, pred_id) in zip(sentence.tokens, predicted_id):
                    ...
                    predicted_tag: str = self.model.tag_dictionary.get_item_for_index(pred_id)
                    if cn and predicted_tag.endswith('-O'):
                        predicted_tag = 'O'
                    token.add_tag('predicted', predicted_tag)

                    # get the gold tag
                    gold_tag: str = token.get_tag(self.model.tag_type)
                    if cn and gold_tag.endswith('-O'):
                        gold_tag = 'O'
        ...
        if evaluation_method == 'accuracy':
            ...
            if dump:
                return score, metric._fp, str(score), att_vec
