...
from bertsota.tagger.corpus import Dictionary, Sentence, Token
from bertsota.tagger.embeddings import TokenEmbeddings, WordEmbeddings, CharLMEmbeddings, StackedEmbeddings
...
class SequenceTagger(nn.Block):
    ...
    def __init__(self,
        ...
        self.rnn = rnn.LSTM(input_size=rnn_input_dim, hidden_size=hidden_size, num_layers=self.nlayers,
                            bidirectional=True)
        if attention:
            self.attention = nlp.model.DotProductAttentionCell(units=512, dropout=0.5)
        else:
            self.attention = None
    ...
    def forward(self, sentences: List[Sentence], dump=False) -> Tuple[nd.NDArray, nd.NDArray, List]:
        """
        ...
        if self.use_rnn:
            ...
            if self.attention:
                ntc = sentence_tensor.transpose((1, 0, 2))
                ctx_vector, weight_vector = self.attention(ntc, ntc)
                sentence_tensor = ctx_vector.transpose((1, 0, 2))
                if dump:
                    att_vec = weight_vector
        ...
        for i, (t, l) in enumerate(zip(tag_list, lengths)):
            tags[i, :l] = t
        if dump:
            return features.transpose([1, 0, 2]), tags, lengths, att_vec
    ...
    def neg_log_likelihood(self, sentences: List[Sentence], tag_type: str):
        ...
        if self.use_crf:
            ...
            return self.softmax(feats, tags)
    ...
    def predict_scores(self, sentence: Sentence, dump=False):
        if dump:
            feats, tags, lengths, att = self.forward([sentence], dump)
        else:
            feats, tags, lengths = self.forward([sentence], dump)
        feats = feats[0]
        tags = tags[0]
        if self.use_crf:
            score, tag_seq = self.viterbi_decode(feats)
        else:
            tag_seq = nd.argmax(feats, 1)
            score = -1
            tag_seq = list(int(y.asscalar()) for y in tag_seq)

        if dump:
            return score, tag_seq, att
